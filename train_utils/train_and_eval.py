...
def evaluate(model, data_loader, device):
    ...
    f1_metric = utils.ClassificationF1Score(num_classes=3)
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    with torch.no_grad():
        for image, audio, target in metric_logger.log_every(data_loader, 100, header):
            image, audio, target = image.to(device), audio.to(device), target.to(device)
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, lr_scheduler, print_freq=10, scaler=None):
    ...
    for image, audio, target in metric_logger.log_every(data_loader, print_freq, header):
        image, audio, target = image.to(device), audio.to(device), target.to(device)
...
def create_lr_scheduler(optimizer,
                        ...
                        warmup_factor=1e-3,
                        end_factor=1e-6):
    print(f"[DEBUG] create_lr_scheduler called with num_step={num_step}, epochs={epochs}")
