...
def init_distributed_mode(args):
    ...
    suppress_print(args.rank == 0)


class ClassificationF1Score(object):
    """
    A class to compute F1 score for classification tasks.
    """

    def __init__(self, num_classes: int, threshold: float = 0.5):
        self.num_classes = num_classes
        self.threshold = threshold
        self.precision_cum = torch.zeros(num_classes)
        self.recall_cum = torch.zeros(num_classes)
        self.num_cum = torch.zeros(num_classes)

    def update(self, pred: torch.Tensor, gt: torch.Tensor):
        """
        Update the F1 score metrics with predictions and ground truth.

        Args:
            pred (torch.Tensor): Predictions, should be raw logits or probabilities.
            gt (torch.Tensor): Ground truth labels, should be in long format.
        """
        # Assuming pred is raw logits and gt is long tensor with class indices
        pred_prob = F.softmax(pred, dim=1)  # Convert logits to probabilities
        pred_labels = torch.argmax(pred_prob, dim=1)  # Get the predicted labels
        
        for class_id in range(self.num_classes):
            # Get binary ground truth and predictions for the current class
            gt_class = (gt == class_id).float()
            pred_class = (pred_labels == class_id).float()

            # Compute true positives, false positives, and false negatives
            tp = torch.sum(pred_class * gt_class).item()
            fp = torch.sum(pred_class * (1 - gt_class)).item()
            fn = torch.sum((1 - pred_class) * gt_class).item()
            ...
            self.precision_cum[class_id] += precision
            self.recall_cum[class_id] += recall
            self.num_cum[class_id] += 1
    ...
    def compute(self):
        ...
        """
        precision_mean = self.precision_cum / (self.num_cum + 1e-8)
        recall_mean = self.recall_cum / (self.num_cum + 1e-8)
        f1_scores = 2 * (precision_mean * recall_mean) / (precision_mean + recall_mean + 1e-8)
        f1_mean = f1_scores.mean().item()
        return f1_mean

    def reduce_from_all_processes(self):
        """
        Reduce F1 score metrics from all processes in a distributed setting.
        """
        if not torch.distributed.is_available():
            return
        if not torch.distributed.is_initialized():
    ...
    def __str__(self):
        ...
        return f'Average F1 Score: {f1_mean:.3f}'
